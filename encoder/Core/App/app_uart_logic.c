...
void Process_UART_Communication(void) {
	...
	// ПРИЕМ ДАННЫХ (из UART в систему) ---
	...
	if (uart_msg_ready) {
		...
		// Парсим "VAL:125" (без знака $, так как коллбэк его отрезает)
		if (sscanf((char*) uart_rx_buffer, "VAL:%d", &value) == 1) {
			...
			// 1. Устанавливаем аппаратный счетчик таймера в новое значение
	...
	// 2. Читаем АКТУАЛЬНОЕ значение из "железа"
